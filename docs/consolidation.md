...
- `lint` reads only what the caller is scoped to, with one exemption: `capsid/schema.md` and `capsid/conventions.md` are returned to every caller as its rules, whatever its scope. No other `capsid` document is read. An edge whose other end is in a namespace the caller is not scoped to is left out of gather and report, and report skips the doc-vs-code drift check when the namespace's repo is outside the caller's repo scope.
