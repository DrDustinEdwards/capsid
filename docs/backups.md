...
- `backups/json/<timestamp>/<table>.json` one JSON dump per table (`TABLES` in `src/backup.ts` is the list). Retention treats the run: kept for 90 days, the 14 most recent complete runs always kept whatever their age, and a run that ages out is deleted whole. A run is complete when it carries `_complete.json`, which is written last and only when the preflight passed; a run that threw mid-export or was refused has none, holds no slot in the floor, and ages out at 90 days like any run. Runs written before the marker existed (those older than the oldest marked run) count as complete, as they did before.
...
3. **The R2 JSON dump**, beyond the 30-day Time Travel window. Wrangler cannot list R2 objects, so take the exact keys from the Cloudflare dashboard or the `json_keys` field of a `/ops/backup` response, then fetch each: `wrangler r2 object get capsid-media/backups/json/<timestamp>/<table>.json --file <table>.json`. Convert each object's `rows` to INSERT statements and follow path 2 from the create step, `documents` first. The same dumps are mirrored off-account in `capsid-backups`, so this path works if the Cloudflare account is gone. The `backups/markdown/` mirror is the last-resort human-readable copy: bodies only, no metadata.
   ...
   The dump is read in a single D1 batch, so its tables describe one instant. `document_versions` and `audit_log` are the exceptions in how they are read, not in what they hold: the batch reads each one's `MAX(id)`, and the rows up to that id are then paged and streamed to R2 as a multipart upload, because reading `document_versions` whole exceeded the isolate's memory from 2026-09-20. Both tables are append-only with an AUTOINCREMENT id, so the bound still describes the batch's instant. `documents` and `jobs` are updated in place, so they are still read whole inside the batch. Each file has the same shape as every other table's. `scripts/restore-rehearsal.mjs` checks the dump weekly and refuses a torn one.
