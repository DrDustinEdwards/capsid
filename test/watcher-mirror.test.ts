...
// The off-account mirror check. A verified POST /backup/credential proves only that
// the mirror started; the run can fail after it. Only a dump proves a backup exists.
//
...
test("anything that is not a dump stamp parses to null, not to a date", () => {
  // A stray directory read as a fresh backup would silence the check permanently.
...
test("REPLAY OF THE 2026-09-09 INCIDENT: stale dump plus a failed run is finding (a)", () => {
  // The credential step can succeed while the run fails, so it is not an input to
  // this check. Newest dump is stale, the run conclusion is failure.
...
test("a fresh dump produces NOTHING, whatever the runs say", () => {
  // The healthy case: a check that fires on a working mirror is useless.
...
test("the window is its own constant and not the local backup's", () => {
  // Two different things measured from two different sides; one constant for both
  // goes wrong silently when the other repo changes its schedule.
...
// Driven through gatherFindings in test/watcher-gather.test.ts: the mirror repo is
// resolved through the namespace's backups label, and an unreadable mirror listing
// posts nothing while a readable empty one posts mirror-no-dump.
