...
// The subscription-mode driver lock and path guard. Subscription mode runs on a
// laptop and creates no run row, so neither the run table's unique index nor the
// Worker's attempt-path guard reaches it.
//
...
//              is best-effort, like the backup lease.
//   the guard  improve_status serves the pattern list, so the driver applies the
//              rules the Worker holds rather than a copy that can drift.
...
test("improve_status SERVES the protected path list, so the driver cannot hold a stale copy", async () => {
  ...
  // Derived in both directions: a pattern added to PROTECTED_PATH_PATTERNS and not
  // served, or served and not in the list, fails.
  assert.deepEqual(
...
test("the served patterns REBUILD to the same verdicts as the Worker's own guard", async () => {
  // A driver reconstructing RegExps from the strings must refuse exactly what the
  // Worker refuses.
  const { env } = harness();
  const status = await improveStatus(env);
  ...
  // The fixture must produce both verdicts, or "they agree" would be true of a guard
...
test("the path guard the driver runs refuses exactly what the Worker refuses", async () => {
  // scripts/path-guard.mjs takes the list improve_status served and the changed
  // paths, and exits nonzero on a hit.
  ...
  // A hit names the path and the reason, because the driver prints it.
