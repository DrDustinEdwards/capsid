  ...
  server.registerTool(
    "jobs",
    {
      ...
      inputSchema: {
        ...
        id: bounded(MAX_JOB_ID).optional().describe('The job id, for claim (optional, to take a specific one), heartbeat, complete, fail, block and resume. For list, narrows to that one job, and a caller holding write then also gets its body.'),
        ...
        reason: bounded(MAX_TITLE).optional().describe('For fail and block: why. For resume: what the human approved, which is what the audit row records and what the next holder of the job reads as resume_note.'),
